...
export default function AdminBookingsPage() {
  ...
  const deleteBooking = async (id: number) => {
    if (confirm("Удалить бронирование навсегда? Отменить нельзя!")) {
      try {
        const res = await fetch(`/api/bookings/${id}?action=force`, { method: "DELETE" });
        if (!res.ok && res.status === 403) {
          alert("У вас нет прав на удаление бронирований");
        }
        fetchBookings();
      } catch (err) {
  ...
  if (bookings.length === 0) return <p className="text-muted-foreground">Нет бронирований</p>;
